...
export const createComment = async (req, res) => {
  try {
    ...
    if (existingComment && existingComment.rating !== null) {
      ...
      await film.save();
      return res.status(200).send("You have already rated this film,but your comment sent.");
    }
...
export const getCommentsForFilm = async (req, res) => {
  try {
    const { filmId } = req.params;
    const comment = await CommentModel.findById(filmId).populate("userId");
    console.log(comment);
    res.send(comment);
  } catch (error) {
    res.status(500).send(error.message);
...
export const deleteComment = async (req, res) => {
  try {
    ...
    await CommentModel.deleteOne({ userId, filmId });
